<tr>    
    <form id="id-<?php echo $id ?>" method="post">
        ...
        <td class="product-price" data-title="Price">R$ <?= number_format($_SESSION['produto'][$id]['preco'], 2, ',', '.') ?></td>
        <td class="product-quantity" data-title="Quantity">
            <div class="quantity">
                <input id="qtdmenos" type="submit" value="-" class="qtde" tipo="menos" name="diminuirquantidade-<?php echo $id ?>">
                <input id="qtd" type="text" disabled="" name="qtde_pedido_item" value="<?= $_SESSION['produto'][$id]['quantidade'] ?>" title="Qty" class="qty" size="4">
                <input id="qtdmais" type="submit" value="+" class="qtde" tipo="mais" name="aumentarquantidade-<?php echo $id ?>">
            </div>
        </td>
        <td class="product-subtotal" data-title="Total">R$ <?= number_format(round($_SESSION['produto'][$id]['precototal'], 2), 2, ',', '.') ?></td>
